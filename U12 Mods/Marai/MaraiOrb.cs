...
public class MaraiOrb : MonoBehaviour
{
    ...
    private MeshRenderer swordPhantomMesh;
    ...
    private void Start()
    {
        ...
        swordRB = swordItem.GetComponentInChildren<Rigidbody>();
        PlayParticles(false);
        swordPhantomMesh = swordItem.GetCustomReference("MaraiPhantom").GetComponent<MeshRenderer>();
        swordMesh = swordItem.GetCustomReference("MaraiMesh").GetComponent<MeshRenderer>();

        //Enable Red outline on sword
        swordPhantomMesh.enabled = true;
        //Set white emission color on sword
    ...
    private void Update()
    {
        ...
        if (retractSword)
            ...
            swordRB.velocity = 10 * (swordTarget.position - swordItem.transform.position);
